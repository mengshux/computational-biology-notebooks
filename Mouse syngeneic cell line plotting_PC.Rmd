...
isr_quantiles %>% dselect(-Group,-ARG_AAS,-GCN2_specific) %>% mutate(Other_ISR=round((PKRi+PERKi+HRIi)/3,digits=1), MDSC=(mMDSC+gMDSC)/2) %>% dselect(-PKR,-PERK,-HRI,-mMDSC,-gMDSC,-PKRi,-PERKi,-HRIi,) %>% mutate(Summary=Common-Other_ISR+MDSC+Pan_AAS) %>% arrange(desc(Summary)) %>% dselect(experimental.group,Common,Other_ISR,MDSC,Pan_AAS,Summary) %>% write.table(file="Simplifed_scores_zzscore_Dec2019.txt",sep="\t", row.names=FALSE, quote=FALSE)
...
crown_unlog %>% dfilter(Gene_Crown %in% c("Pvr","Nectin","Cd112","ectin") )
...
crown_unlog %>% dfilter(Gene_Crown %in% c("Nectin2") ) %>%
  gather(Syngeneic_model, FPKM, -Gene_Crown) %>%  mutate(Syngeneic_model = str_replace(Syngeneic_model,"_Crown","")) %>%
  ggplot(aes(x=reorder(Syngeneic_model,-FPKM), y=FPKM, ymin=0, ymax=FPKM)) +
  geom_point() +
  geom_linerange() +
  theme(axis.title = element_text(size=12), axis.text.y = element_text(size=14), axis.text.x = element_text(angle=45,size=12,hjust=1))
