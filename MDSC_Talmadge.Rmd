...
load("/Volumes/Picard/FLX/scRNA_data/MDSC_Talmadge/Cellranger3.1.0_batch3/batch3b.Robj")
...
rownames(cancer_pdac3$`Gene Expression`)
load("/Volumes/Picard/FLX/scRNA_data/MDSC_Talmadge/Cellranger3.1.0_batch3/batch3c.Robj")
#batch4
...
batch4$CITE@data@Dimnames[[1]]
new_CITE_names <- rownames(batch4$CITE) %>% str_replace("-human","_CITE")
batch4$CITE@data@Dimnames
  Dimnames[[1]] <- new_CITE_names
...
GetAssayData(pbmc2, cells= cells_n15, slot="counts")
...
unique(batch4B$orig.ident)
  p1 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="pdac1"),cols.highlight="darkorchid2",sizes.highlight=0.2) + NoLegend()
  p2 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="pdac2"),cols.highlight="darkorchid2",sizes.highlight=0.2) + NoLegend()
  p3 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="pdac3"),cols.highlight="darkorchid2",sizes.highlight=0.2) + NoLegend()
  n1 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="pbmc1"),cols.highlight="dodgerblue2",sizes.highlight=0.2) + NoLegend()
  n2 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="pbmc2"),cols.highlight="dodgerblue2",sizes.highlight=0.2) + NoLegend()
  n3 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="pbmc3"),cols.highlight="dodgerblue2",sizes.highlight=0.2) + NoLegend()
  a1 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="aml1"),cols.highlight="firebrick1",sizes.highlight=0.05) + NoLegend()
  a3 <- DimPlot(batch4B, cells.highlight = WhichCells(batch4,expression= orig.ident =="aml3"),cols.highlight="firebrick1",sizes.highlight=0.05) + NoLegend()
...
load("/Volumes/Picard/FLX/Reference_tables/gMDSC_Talmadge_sig_m.rdata")
...
DoHeatmap(batch4, features= top.markers$gene) + NoLegend()
...
FeaturePlot(batch4,slot = "data",features = c("CD11b-CITE","CD11c-CITE","CD14-CITE","CD33-CITE","BPI","CD15-CITE","CSF-1R-CITE","CD34-CITE","FCGR3B","CD45RA-CITE","CD45RO-CITE"))
rownames(batch3c$CITE@data) %>% table 

FeaturePlot(batch4,slot = "counts",features = c("CD4-CITE"), cols=c("grey80","red"),max.cutoff=100, min.cutoff=70)
...
FeaturePlot(batch4,slot = "data",cols=c("grey90","red"),features = c("IL8","CXCL3","CCL7","IL1R1","IL6","CCL17","CCL22","CD86","CD24"), ncol=3) 
ggsave("mRNA_CITE_comparison_myeloid_counts.jpg", width=20, height=10, dpi=300, plot= last_plot(), units = "in")

FeaturePlot(batch4,slot = "data",cols=c("grey90","red"),features = c("CXCL3","IL1R1","IL6","CCL17","CCL22","CD86","CD24","IFNG","IL12","IL5","TNF","IL4"), ncol=3) 
ggsave("mRNA_CITE_comparison_myeloid_counts.jpg", width=20, height=10, dpi=300, plot= last_plot(), units = "in")
```
...
FeaturePlot(batch4B,reduction="umap",slot ="data",cols=c("grey90","red"),features = c( "CCR4","CCR8","FOXP3"), ncol=3, max.cutoff = 1) 
#ggsave("CITE_feature_counts.jpg", width=20, height=10, dpi=300, plot= last_plot(), units = "in")
rownames(batch1$CITE)
```
...
```{r}
batch4_myeloid <- subset(batch4, idents=c(2,3,5,8,21,13,20,10,7,11,12,15,25,22)) # 10, 9, 16, 28 are dying population, 10 was kept in 
save(batch4_myeloid, file="batch4_myeloid.Robj") 
...
```{r}
rna_matrix <- as.matrix(batch3c$RNA@data)
save(rna_matrix,file = "batch3c_data_matrix.rdata")
load("batch3b_data_matrix.rdata")
...
fData <- data.frame(gene_short_name = row.names(my.data), row.names = row.names(my.data))
fd <- new('AnnotatedDataFrame', data = fData)

#Construct monocle cds
CT26_monocle <- newCellDataSet(my.data,
                         phenoData = pd,
                         featureData = fd,
...
Seurat 3 into Monocle 3 is not supported
Seurat 2 into Monocle 3 is supported
#######################
go to Terminal and install gcc with "brew install gcc". Otherwise leidenbase will exit with error "make: gfortran: No such file of directory"
```{r}
...
Seurat 3 into Monocle 3
```{r}
#Load Seurat object

#Extract data, phenotype data, and feature data from the SeuratObject
my.data <- as(as.matrix(CT26_immunome@assays$RNA@data), 'sparseMatrix')

pd <- new('AnnotatedDataFrame', data = CT26_immunome@meta.data)
...
```{r}
CT26_monocle <- estimateSizeFactors(CT26_monocle)
CT26_monocle<- estimateDispersions(CT26_monocle)
```

Preprocess data to find pricipal components
```{r}
CT26_monocle <- preprocess_cds(CT26_monocle) # this does not exist as a function in monocle 3
CT26_monocle <- reduceDimension(CT26_monocle, reduction_method = 'DDRTree') # changed from UMAP

CT26_monocle <- clusterCells(CT26_monocle,method = 'louvain',
                        res = 1e-6,
                        louvain_iter = 1,
                        verbose = T)

plot_cell_clusters(CT26_monocle, color_cells_by = "partition")
```
...
cell_type_colour <- c("0" = "#9400D3", #purple
                      ...
                      "15" ="#FF0000", #red
                      "16" = "#00008B") #navyblue
plot_cell_trajectory(myeloid_m, label_groups_by_cluster=FALSE,  color_cells_by = "cell.type")
plot_cell_trajectory(myeloid_m , 
                     use_color_gradient = TRUE,
                     show_backbone=TRUE, alpha = 0.9)  +
                    theme(legend.position = "bottom") 

markers = c("TGFB1","CD276","SPP1","LILRB2","CXCL10"), 
