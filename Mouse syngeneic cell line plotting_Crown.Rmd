...
ISR_markers <- c(Common,PRK,PERK_try,HRI_sub,gMDSC_sig_m, mMDSC_sig_m,"Eif2ak4", "Ido1","Arg1")
ISR_markers <- c(Common,PRK,PERK_try,HRI_sub,"Eif2ak4", "Ido1","Arg1")
...
melt_med_sig_cor %>% ggplot(aes(x=Var1, y=Var2, fill=value, label=value)) + geom_tile(color = "white")+
 ...
 coord_fixed() +
ggsave("Crown_Sig_corr_median_heatmap_Sep1.jpg", width=6, height=6, dpi=600, plot= last_plot(), units = "in")  
...
ISR_median <- ISR_s_t %>% mutate(PRKi= ifelse(PRK<0,0,PRK), PERKi=ifelse(PERK<0,0,PERK), HRIi=ifelse(HRI<0,0,HRI)) %>% 
  mutate(GCN2_specific = Common - PRKi - PERKi - HRIi) %>% mutate(Group=`experimental group`)  %>% group_by(Group) %>% summarise_if(is.double,median) %>% dfilter(Group %like% "_T$") %>% arrange(desc(GCN2_specific)) %>% dselect(Group,Group,GCN2s,Common,PRK,PERK,HRI,GCN2_specific,AAS,gMDSC,mMDSC) 

ISR_median %>% write.table(file="ISR_scores_grouped_ZZscore_Sep10.cdt",sep="\t",row.names=FALSE,quote=TRUE)
```
...
ISR_s_t %>% mutate(PRKi= ifelse(PRK<0,0,PRK), PERKi=ifelse(PERK<0,0,PERK), HRIi=ifelse(HRI<0,0,HRI)) %>% mutate(GCN2_specific = Common - PRKi - PERKi - HRIi)  %>% arrange(desc(GCN2_specific)) %>% mutate(Cell_line=lines, Cellline=lines) %>% dselect(-PRKi,-PERKi, -HRIi) %>% dselect(Cell_line,Cellline,GCN2s,Common,PRK,PERK,HRI,GCN2_specific,AAS,gMDSC,mMDSC) %>% as.data.table %>% write.table(file="ISR_scores_grouped_ZZscore_CrownBio_GCN2_s.cdt",row.names = FALSE,sep="\t")
