...
load(file="/Volumes/Picard/FLX/GCN2/ISR_data/RNA_seq/GCN2_PERK_kallisto.rdata")
``` 
...
save(ISR_norm,file="ISR_norm.rdata")
...
save(ISR_counts, file="ISR_counts.rdata")
```
...
Zak= MAP3k20
```{r}
ISR_gene <- ISR_norm %>% group_by(Symbol) %>% mutate_if(is.numeric, sum_mx) %>% dselect(-Gene) %>% unique

ISR_gene %>% dfilter(Symbol %in% c("Map3k20","Mapk14")) %>% gather(Condition, mRNA_signal,-Symbol) %>% separate(Condition, c("Genotype","Treatment","Rep"), sep="[_.]", remove=FALSE) %>% mutate(Treatment= factor(Treatment, levels=c("NT","DMSO","AAS","Thap")), Genotype = factor(Genotype, levels=c("WT","G","P"))) %>% dfilter(Genotype != "P", Treatment != "Thap") %>% ggplot(aes(x=interaction(Treatment,Genotype),y=mRNA_signal,colour=Genotype)) + geom_boxplot() + geom_point() + facet_wrap(~Symbol, scales="free_y") + theme(axis.text.x=element_text(angle=90, size=14)) 

ISR_gene %>% dfilter(Symbol %in% c("Bid","Bak1","Casp3","Casp7")) %>% gather(Condition, mRNA_signal,-Symbol) %>% separate(Condition, c("Genotype","Treatment","Rep"), sep="[_.]", remove=FALSE) %>% mutate(Treatment= factor(Treatment, levels=c("NT","DMSO","AAS","Thap")), Genotype = factor(Genotype, levels=c("WT","G","P"))) %>% dfilter(Genotype != "P", Treatment != "Thap") %>% ggplot(aes(x=interaction(Treatment,Genotype),y=mRNA_signal,colour=Genotype)) + geom_boxplot() + geom_point() + facet_wrap(~Symbol, scales="free_y") + theme(axis.text.x=element_text(angle=90, size=14)) 

ISR_gene %>% dfilter(Symbol %in% c("Bcl2")) %>% gather(Condition, mRNA_signal,-Symbol) %>% separate(Condition, c("Genotype","Treatment","Rep"), sep="[_.]", remove=FALSE) %>% mutate(Treatment= factor(Treatment, levels=c("NT","DMSO","AAS","Thap")), Genotype = factor(Genotype, levels=c("WT","G","P"))) %>% dfilter(Genotype != "P", Treatment != "Thap") %>% ggplot(aes(x=interaction(Treatment,Genotype),y=mRNA_signal,colour=Genotype)) + geom_boxplot() + geom_point() + facet_wrap(~Symbol, scales="free_y") + theme(axis.text.x=element_text(angle=90, size=14)) 
...
Gs <- G_spec %>% dfilter(log2FoldChange >=2) %>% dselect(Gene)
Gs[,1]
...
```{r}
![DE schematic](/Volumes/Picard/FLX/GCN2/ISR_data/DE_plan.jpg)
```
