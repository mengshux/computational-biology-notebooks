---
title: "HPK1_OTI_22054"
author: "MX"
date: "5/3/2021"
output: html_document
---

Need to install the latest DESeq to work with updated R 4.0.5
DeSEQ 1.28.1 does not work anymore after the R upgrade
Upgraded to DESeq2_1.30.1  
```{r}
library(limma)
library(tidyverse)
library(dplyr)
...
BiocManager::version() #3.11 this is out of date
if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")
BiocManager::install(version = "3.12")
```

Install Enrichr for R
```{r}
library(devtools)
...
Save and Load data
```{r}
save(tpm_gene,file="/Volumes/Picard/FLX/HPK1/Listeria_OVA_054_HPK1/tpm_gene.rdata") # normalized and summed by gene

load("/Volumes/Picard/FLX/HPK1/Listeria_OVA_054_HPK1/tpm_gene.rdata")
tpms
```

Read in the data
```{r}
file_names <- list.files("/Volumes/Picard/FLX/HPK1/Listeria_OVA_054_HPK1/Kallisto_output/", pattern=".tsv") %>% lapply(function(i) {strsplit(i,"_abundance\\.")[[1]][1]})
files <- base::list.files("/Volumes/Picard/FLX/HPK1/Listeria_OVA_054_HPK1/Kallisto_output/", pattern=".tsv", full.names=TRUE)
...
load("tpm_gene.rdata")
tpm_gene %>% write.table(file="Listeria_F22054_norm_TPM.txt", sep="\t",row.names = FALSE,quote=F)
...
files <- base::list.files("/Volumes/Picard/FLX/HPK1/Listeria_OVA_054_HPK1/Kallisto_output/", pattern=".tsv", full.names=TRUE)
gencode_symbol <- read.table("/Volumes/Picard/FLX/Reference_tables/GRCm38_GENCODE2SYMBOL.txt",sep="\t",header=TRUE) %>% data.frame
names <- base::list.files("/Volumes/Picard/FLX/HPK1/Listeria_OVA_054_HPK1/Kallisto_output/", pattern=".tsv") %>% lapply(function(i) {strsplit(i,"_abundance\\.")[[1]][1]}) %>% unlist
...
meta <- data.frame(Sample= names) %>% mutate(Treatment= case_when(
  grepl("D3C",Sample) ~ "Vehicle_Day3",
  grepl("D3T",Sample) ~ "F22054_100mpk_Day3",
  grepl("D7C",Sample) ~ "Vehicle_Day7",
  grepl("D7T",Sample) ~ "F22054_100mpk_Day7",
)) %>% mutate(Day = if_else(grepl("D3",Sample),"Day_3","Day_7")) %>% mutate(Treatment=factor(Treatment,levels=c("Vehicle_Day3","F22054_100mpk_Day3","Vehicle_Day7","F22054_100mpk_Day7"))) 

OT1 <- DESeqDataSetFromTximport(ot1,
                                   colData = meta,
                                   design= ~ Treatment)
OT1@assays@data$counts
save(OT1,file="DESeq2_Listeria_054.rdata")
load(file="DESeq2_Listeria_054.rdata")
#OT1 <- DESeq(OT1)
rld <- rlog(OT1)
plotPCA(rld,intgroup=c("Treatment")) + scale_color_manual(values= c("#0A95A7", "#E5541B","#5B666F","#FE9F33"))
...
aload %>% arrange(-PC1) %>% top_n(100) %>% rownames %>% data.frame
```
...
```{r}
OT1$Treatment <- relevel(OT1$Treatment, ref = "Vehicle_Day3")

OT1 <- DESeq(OT1)

resultsNames(OT1)

Vehicle_Day7_vs_Day3_UP <- results(OT1, name="Treatment_Vehicle_Day7_vs_Vehicle_Day3") %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(padj <= 0.01,log2FoldChange >= 0.5) %>% dfilter(!grepl("Gm|Rik",Gene)) %>% arrange(padj) %>% pull(Gene)
Vehicle_Day7_vs_Day3_UP %>% write.table(file="Vehicle_Day7_vs_Day3_UP.txt", sep="\t",row.names = FALSE,quote=F)

Vehicle_Day7_vs_Day3_DN <- results(OT1, name="Treatment_Vehicle_Day7_vs_Vehicle_Day3") %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(padj <= 0.01,log2FoldChange <= -0.5) %>% dfilter(!grepl("Gm|Rik",Gene)) %>% arrange(padj) %>% pull(Gene)
Vehicle_Day7_vs_Day3_DN %>% write.table(file="Vehicle_Day7_vs_Day3_DN.txt", sep="\t",row.names = FALSE,quote=F)

Res_054_vs_Vehicle_Day3_sig <- results(OT1, name="Treatment_F22054_100mpk_Day3_vs_Vehicle_Day3") %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(padj <= 0.01, abs(log2FoldChange) >= 0.5) %>% dfilter(!grepl("Gm|Rik",Gene)) %>% arrange(-log2FoldChange)  %>% pull(Gene)
Res_054_vs_Vehicle_Day3_sig %>% write.table(file="F22054_100mpk_Day3_vs_Vehicle_Day3_sig.txt", sep="\t",row.names = FALSE,quote=F)

Res_054_vs_Vehicle_Day3_all <- results(OT1, name="Treatment_F22054_100mpk_Day3_vs_Vehicle_Day3") %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(!grepl("Gm|Rik",Gene)) %>% arrange(padj)
Res_054_vs_Vehicle_Day3_all %>% write.table(file="F22054_100mpk_Day3_vs_Vehicle_Day3_all.txt", sep="\t",row.names = FALSE,quote=F)

Res_054_vs_Vehicle_Day3_DN <- results(OT1, name="Treatment_F22054_100mpk_Day3_vs_Vehicle_Day3") %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(padj <= 0.01,log2FoldChange <= -0.5) %>% dfilter(!grepl("Gm|Rik",Gene)) %>% arrange(padj) %>% pull(Gene)
Res_054_vs_Vehicle_Day3_DN %>% write.table(file="F22054_100mpk_Day3_vs_Vehicle_Day3_DN.txt", sep="\t",row.names = FALSE,quote=F)

Res_054_vs_Vehicle_Day7_UP <- results(OT1, name="Treatment_F22054_100mpk_Day7_vs_Vehicle_Day7") %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(padj <= 0.01,log2FoldChange >= 0.5) %>% dfilter(!grepl("Gm|Rik",Gene)) %>% arrange(padj) # %>% pull(Gene)
Res_054_vs_Vehicle_Day7_UP  %>% write.table(file="F22054_100mpk_Day7_vs_Vehicle_Day7_UP.txt", sep="\t",row.names = FALSE,quote=F)

```

Enrichr

Bioplanet 2019 is not available thru the R tool
```{r}
setEnrichrSite("Enrichr")
websiteLive <- TRUE
dbs <- listEnrichrDbs()
ontologies <- c("BioCarta_2016","BioPlanet_2019","NCI-Nature_2016","Reactome_2016","Gene_Perturbations_from_GEO_up","Gene_Perturbations_from_GEO_down","GeneSigDB","Human_Gene_Atlas","PPI_Hub_Proteins","GO_Biological_Process_2018","WikiPathways_2016","KEGG_2016","Panther_2016","Jensen_COMPARTMENTS","Jensen_DISEASES","Mouse_Gene_Atlas","KEGG_2019_Mouse")
if (websiteLive) {
    enriched <- enrichr(Res_054_vs_Vehicle_Day3_DN, ontologies)
}

if (websiteLive) enriched[["GO_Biological_Process_2018"]] %>% arrange(P.value) %>% slice_max(order_by=-P.value,n=8) %>% dselect(Term,P.value,Adjusted.P.value,Genes) %>% View # write.table("clipboard",sep="\t")
```
...
EnhancedVolcano(results(OT1, name="Treatment_F22054_100mpk_Day7_vs_Vehicle_Day7"), #"Name_F019324_100mpk_Day7_vs_Vehicle_Day7"
...
TCR pathway
c("Cd28","Cd3a","Lck","Zap70","Lcp2","Prdm1")
Proliferation
c("Mki67","Slamf7")
...
Interesting_markers_m <- tpm_z %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(Gene %in% c("Cd274")) %>% tibble::column_to_rownames("Gene") %>% as.matrix
...
Interesting_markers_m <- tpm_z %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(Gene %in% c("Kit","Ptpn11","Gab2","Nras","Kras","Braf","Map4k1","Map3k1","Map2k1","Mapk3","Nf1","Spred1","Pik3ca","Akt1")   ) %>% tibble::column_to_rownames("Gene") %>% as.matrix
...
library(ggbeeswarm)
Flow_markers <- tpm_z %>% data.frame %>% tibble::rownames_to_column("Gene") %>% dfilter(Gene %in% c("Tnf","Ifng","Gzmb")) 
...
```
```{r}
tpm_gene %>% dfilter(Symbol %in% c("Cdk4","Top2a","Cenpe","Polr1c","Rpl4","Rpl30","Rpl3","Cox7b","Eif4a1","Cox17","Ndufb9","Atp5j")) %>% dselect(Symbol,starts_with("D3")) %>% gather(Sample, mRNA_TPM,-Symbol) %>% mutate(Treatment=c(rep("Vehicle",60),rep("22054",60) ) ) %>% mutate(Treatment=factor(Treatment, levels=c("Vehicle","22054"))) %>% mutate(Gene=factor(Symbol, levels=c("Cdk4","Top2a","Cenpe","Polr1c","Rpl4","Rpl30","Rpl3","Cox7b","Eif4a1","Cox17","Ndufb9","Atp5j"))) %>%
...
tpm_gene %>% dfilter(Symbol %in% c("Cdk4","Top2a","Cenpe","Polr1c","Rpl4","Rpl30","Rpl3","Cox7b","Eif4a1","Cox17","Ndufb9","Atp5j")) %>% dselect(Symbol,starts_with("D7")) %>% gather(Sample, mRNA_TPM,-Symbol) %>% mutate(Treatment=c(rep("Vehicle",60),rep("22054",60) ) ) %>% mutate(Treatment=factor(Treatment, levels=c("Vehicle","22054"))) %>% mutate(Gene=factor(Symbol, levels=c("Cdk4","Top2a","Cenpe","Polr1c","Rpl4","Rpl30","Rpl3","Cox7b","Eif4a1","Cox17","Ndufb9","Atp5j"))) %>%
  ggplot(aes(x=Treatment, y=mRNA_TPM)) + facet_wrap(~Gene, scales="free_y") + geom_boxplot(aes(fill=Treatment)) + geom_quasirandom() + theme_bw() + scale_fill_manual(values=c("#0A95A7","#E5541B"))
```
Cell cycle markers across Day 3 and Day 7

```{r fig.height=2.5, fig.width=8}
tpm_gene %>% dfilter(Symbol %in% c("Cdk4","Top2a","Cox17")) %>% gather(Sample, mRNA_TPM,-Symbol) %>% mutate(Treatment=c(rep("Vehicle_Day3",15),rep("22054_Day3",15),rep("Vehicle_Day7",15),rep("22054_Day7",15) ) ) %>% mutate(Treatment= factor(Treatment, levels=c("Vehicle_Day3","22054_Day3","Vehicle_Day7","22054_Day7"))) %>%
...
Box and Whisker plots of TPM data

```{r fig.height=2, fig.width=6}
library(ggbeeswarm)
tpm_gene %>% data.frame %>% dfilter(Symbol %in% c("Tnf","Ifng")) %>% gather(Sample, TPM_norm,-Symbol) %>% left_join(meta, by="Sample") %>% ggplot(aes(Treatment,TPM_norm, colour=Treatment)) + geom_boxplot(alpha=0.1) + geom_beeswarm() + facet_wrap(~Symbol, scale="free_y") + theme(axis.text.x = element_text(angle=90,size=12, vjust=0.5, hjust=1),strip.text=element_text(size=12)) + theme_bw()
```
...
library(ggbeeswarm)
tpm_gene %>% data.frame %>% dfilter(Symbol %in% c("Casp1","Casp4","Bid")) %>% gather(Sample, TPM_norm,-Symbol) %>% left_join(meta, by="Sample") %>% ggplot(aes(Treatment,TPM_norm, colour=Treatment)) + geom_boxplot(alpha=0.1) + geom_beeswarm() + facet_wrap(~Symbol, scale="free_y") + theme(axis.text.x = element_text(angle=90,size=12, vjust=0.5, hjust=1),strip.text=element_text(size=12)) + theme_bw()
```

Exhaustion scatter plot
...
library(ggbeeswarm)
tpm_gene %>% data.frame %>% dfilter(Symbol %in% c("Entpd1","Tigit","Havcr2","Nr4a1","Nr4a2","Nr4a3","Eomes","Cd69","Lag3","Cd274")) %>% gather(Sample, TPM_norm,-Symbol) %>% left_join(meta, by="Sample") %>% mutate(Symbol= factor(Symbol,levels=c("Entpd1","Tigit","Nr4a1","Nr4a2","Nr4a3","Eomes","Cd69","Lag3","Cd274","Havcr2"))) %>% ggplot(aes(Treatment,TPM_norm, colour=Treatment)) + geom_boxplot(alpha=0.1) + geom_beeswarm() + facet_wrap(~Symbol, scale="free_y", ncol=5) + theme(axis.text.x = element_text(angle=90,size=12, vjust=0.5, hjust=1),strip.text=element_text(size=12)) + theme_bw()

test_gene = "Nr4a3"
t.test(x=tpm_gene %>% dfilter(Symbol == test_gene) %>% .[,2:6] %>% unlist, y=tpm_gene %>% dfilter(Symbol == test_gene) %>% .[,7:11] %>% unlist, paired=F, alternative="two.sided")
```
