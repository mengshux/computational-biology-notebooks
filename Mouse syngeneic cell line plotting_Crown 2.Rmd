...
syngeneic_f <- syngeneic %>% dfilter(r_mean >= 3, r_sd >= 50)  
```
...
Syngeneic_batchcorr <- sva::ComBat(dat=as.matrix(syngeneic_f[,2:64]), batch=batches, mod=model0, par.prior = TRUE,prior.plots=FALSE)  #, par.prior= TRUE, prior.plots = TRUE 
boxplot(syngeneic_f[,2:60])
boxplot(Syngeneic_batchcorr)
rownames(Syngeneic_batchcorr) <- syngeneic_f$Gene_Crown
library(limma)
...
ISR_markers <- c(GCN2,Common,PRK,PERK,HRI_sub,gMDSC_sig_m, mMDSC_sig_m,"Eif2ak4", "Ido","Arg1")
