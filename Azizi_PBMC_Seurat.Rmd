...
save(regDCm, file='azizi_regDC_Monocle.RData')
load('azizi_regDC_Monocle.RData') # don't pass to anything
```
...
VlnPlot(object=azizi_pbmc, features.plot=c("nGene","GAPDH","CD276","HLA.DRA"),nCol=2,point.size.use = 0.05, x.lab.rot = TRUE) +
ggsave("Azizi_PBMC_Doublets.jpg", dpi= 600, width=14, height=6, units= "in", plot=last_plot())
```
...
VlnPlot(regDC, features.plot = c("nUMI","nGene","CD276"))
reg <- SubsetData
FeaturePlot(object=regDC, features.plot = c("SLAMF8","CD59","S100A9","LYZ","FCGR3A","LST1","DAB2","IL3RA","CD276"), cells.use= "1", 
cols.use=c("grey90","blue"),no.legend=FALSE,reduction.use="umap") 
```
...
FeaturePlot(object=myeloid, features.plot = c("TGFB1","IL10","ITGAM","IL1B","CD276","FAS", "PDCD1","PDCD1LG2", "IL6"), cols.use=c("grey90","blue"),no.legend=FALSE,reduction.use="umap") 
...
DCsubgroup2 <- SubsetData(myeloid, ident.use=c("0. DC1","2. Immature DC","4. DC1","7. DC5","11. DC1"), subset.raw= TRUE, do.clean= FALSE )
...
FeaturePlot(DCsubgroup, features.plot = c("CD59","S100A9","LYZ","FCGR3A","DAB2","CD276"), cols.use=c("grey90","blue"),no.legend=FALSE,reduction.use="umap") 

ggsave("Azizi_DCsubgroup_DCMarkers.jpg", width= 10, height=7, dpi=600, plot= last_plot(), units = "in")
VlnPlot(DCsubgroup, features.plot = c("HLA.DPB1","TGFB1","ITGAM","ITGAX", "PDCD1","CD276"), x.lab.rot = TRUE, nCol=3) #,use.raw=TRUE
#ggsave("Azizi_DCsubgroup_Vln.jpg", width= 12, height=8, dpi=600, plot= last_plot(), units = "in")
```
...
#DC lineage markers
plot_cell_trajectory(regDCm, 
                     markers = c("SLAMF8","CD59","S100A9","LYZ","FCGR3A","LST1","DAB2","IL3RA","CD276"), use_color_gradient = TRUE,
                     show_backbone=TRUE, alpha = 0.9)  +
                    theme(legend.position = "bottom") + scale_color_gradient(low="grey90", high="blue")


ggsave("Azizi_regDC_Supercluster_DCMarkers.jpg", plot = last_plot(), dpi=400, height= 12, width = 15, units = "in")
...
This eliminates branching points and makes a cleaner Moran statistic
Go Back to Monocle Pseudotime analysis and come back to this point: do a musical CODA
Also do a CODA back to the Seurat analysis
```{r}
